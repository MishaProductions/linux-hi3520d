...
static int hirtc_get_alarm(struct rtc_time *compositetime)
{
...
static int hirtc_get_time(struct rtc_time *compositetime)
{
	unsigned char dayl, dayh;
	unsigned char second, minute, hour;
	unsigned long seconds = 0;
...
static int hirtc_set_time(struct rtc_time *compositetime)
{
	unsigned char ret;
	unsigned int days;
	unsigned long seconds = 0;
	...
	spi_rtc_write(RTC_LR_S, compositetime->tm_sec);
	spi_rtc_write(RTC_LR_M, compositetime->tm_min);
	spi_rtc_write(RTC_LR_H, compositetime->tm_hour);
...
static long hi_rtc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	unsigned long flags;
	int ret;
	...
	case RTC_ALM_READ:
		hirtc_get_alarm(&real_time);
		if (copy_to_user((void *)arg, &real_time, sizeof(real_time)))
			return -EFAULT;
		break;

	case HI_RTC_ALM_SET:
		if (copy_from_user(&time, (struct rtc_time_t *)arg, sizeof(time)))
			return -EFAULT;
	...
	case RTC_RD_TIME:
		...
		hirtc_get_time(&real_time);

		ret = copy_to_user((void *)arg, &real_time, sizeof(real_time));
		if (ret) {
			printk(KERN_INFO "rtc: copy_to_user failure\n", cmd);
			return -EFAULT;
		}
		break;
	...
	case RTC_SET_TIME:
		...
		ret = copy_from_user(&real_time, (struct rtc_time_t *)arg, sizeof(real_time));
		if (ret) {
			return -EFAULT;
		}

		hirtc_set_time(&real_time);
		break;
	...
	case HI_RTC_REG_SET:
		ret = copy_from_user(&reg_info, (struct reg_data_t *)arg, sizeof(reg_info));
		if (ret) {
			return -EFAULT;
		}
	...
	case HI_RTC_SET_TEMP_MODE:
		...
		ret = copy_from_user(&temp_mode, (struct reg_temp_mode_t *)arg, sizeof(temp_mode));
		if (ret) {
			return -EFAULT;
		}
	...
	default:
		printk(KERN_WARNING "invaild rtc command: %d\n", cmd);
		return -EINVAL;
	}
...
static const struct of_device_id rtc_of_match[] = {
	{
		.compatible = "hisilicon-rtc",
	},
	{},
};
static struct miscdevice rtc_char_driver = { .minor = RTC_MINOR, .name = RTC_NAME, .fops = &hi_rtc_fops };
...
static int __init rtc_init(void)
{
	...
	//ret = request_irq(RTC_IRQ, &rtc_alm_interrupt, 0, "RTC Alarm", NULL);
	//if (0 != ret) {
	////	HI_ERR("hi3520D rtc: failed to register IRQ(%d)\n", RTC_IRQ);
	//	goto RTC_INIT_FAIL1;
	//}

	//ret = request_irq(RTC_TEMP_IRQ_NUM, &rtc_temperature_interrupt,
	///		  //IRQF_SHARED,
