...
static void hieth_adjust_link(struct net_device *dev)
{
	...
	if (stat != ld->link_stat) {
		if (ld->phy->link == 0) {
			...
			writel(0x0, (void*)(ld->iobase + 0x490));
			writel(0x0, (void*)(ld->iobase + 0x484));
		} else {
			writel(0x1, (void*)(ld->iobase + 0x490));
			...
			if (eee_available > NO_EEE) {
				...
				if (eee_available == MAC_EEE) {
					writel(0x10, (void*)(ld->iobase + 0x484));
					/* enable intr */
					writel(0x1f,(void*) (ld->iobase + 0x480));
				} else
					writel(0x0, (void*)(ld->iobase + 0x484));
...
static irqreturn_t hieth_net_isr(int irq, void *dev_id)
{
	...
	if (eee_available == MAC_EEE) {
		...
		eee_ints = readl((void*)(ld->iobase + 0x480));
		/* get ready for LPI */
		if (eee_ints & TX_LPI_COND) {
			/* clear eee_intr status */
			writel(0x10, (void*)(ld->iobase + 0x480));
			data = readl((void*)(ld->iobase + 0x488));
			/* tx_lpi_assert enable */
			data |= 0x2;
			writel(data, (void*)(ld->iobase + 0x488));   /*LPI entry*/
		}

		if (eee_ints & TX_ENTRY_LPI) {
			/* clear eee_intr status */
			writel(0x1, (void*)(ld->iobase + 0x480));
		}

		if (eee_ints & TX_LEAVE_LPI) {
			/* clear eee_intr status */
			writel(0x2, (void*)(ld->iobase + 0x480));
		}
	}
#endif
...
static int hieth_net_open(struct net_device *dev)
{
	...
	writel(data, (void*)(ld->iobase + 0x210));
	...
	if (hiphyinfo) {
		...
		if (eee_available > NO_EEE) {
			...
			if (eee_available == MAC_EEE) {
				...
				writel(0x35, (void*)(ld->iobase + 0x494));
				writel(0x1e03e8, (void*)(ld->iobase + 0x48c));
				writel(0xf42400, (void*)(ld->iobase + 0x488));
				data = readl((void*)(ld->iobase + 0x490));
				data |= 0x2;
				writel(data, (void*)(ld->iobase + 0x490));
