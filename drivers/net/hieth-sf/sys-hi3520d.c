...
#define tmp_mdio_ready() \
	(readl((void*)IO_MDIO_RWCTRL) & (0x1 << 15))

	unsigned int reg_value = 0;
	/* select page 7 */
	do {
		reg_value = 0x0007235f;
		writel(reg_value, (void*)IO_MDIO_RWCTRL);
		udelay(10);
...
#ifdef CONFIG_HIETH_INNER_PHY
	val = readl((void*)(HIETH_SYSCTRL_BASE + HIETH_PER24));
	val &= ~(HIETH_PEPHY_ADDR_MASK);
	val |= (HIETH_PEPHY_ADDR_MASK & CONFIG_HIETH_PHYID_U);
	writel(val, (void*)(HIETH_SYSCTRL_BASE + HIETH_PER24));
#endif
	val = readl((void*)(HIETH_SYSREG_BASE + HIETH_SYSREG_REG));
	val |= (1 << 1);
	writel(val, (void*)(HIETH_SYSREG_BASE + HIETH_SYSREG_REG));
}
...
#ifdef CONFIG_HIETH_INNER_PHY
	unsigned int tmp = 0;
	tmp = readl((void*)(HIETH_SYSREG_BASE + HIETH_SYSREG_REG));
	tmp |= (1 << 2);
	writel(tmp, (void*)(HIETH_SYSREG_BASE + HIETH_SYSREG_REG));
	msleep(200);
	tmp = readl((void*)(HIETH_SYSREG_BASE + HIETH_SYSREG_REG));
	tmp &= ~(1 << 2);
	writel(tmp, (void*)(HIETH_SYSREG_BASE + HIETH_SYSREG_REG));
